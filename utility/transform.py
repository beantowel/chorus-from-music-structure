...
from models.seqRecur import ssmStructure_sr
from utility.common import extractFunctions, cliqueGroups
from utility.GraphDitty.SongStructure import ssmFromOpt, Opt
        ...
        for lab, _ in DATASET_LABEL_DIC.items():
            ...
            for xbegin, xend in intvs:
                ...
                for ybegin, yend in intvs:
                    ...
                    target[xlower:xhigher,
                           ylower:yhigher] = SSM_SEMANTIC_LABEL_DIC[lab]

        sample['input'] = self.logSSM(ssm)
...
class ExtractCliques(BaseTransform):
    ...
    def preprocessor(self, wavPath, sr=SAMPLE_RATE):
        ...
        with open(pklPath, 'rb') as f:
            ssm = pickle.load(f)
        times, ssm = (ssm['times'], self.tf.logSSM(ssm['ssm'])[0])
        cliques = ssmStructure_sr((times, ssm))
