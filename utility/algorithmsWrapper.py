...
def matchCliqueLabel(times, boundaries, cliques, dataset, idx):
    intervals = np.array([(times[i], times[j])
...
class AlgoSeqRecur():
    ...
    def __init__(self):
        self.clf = ChorusClassifier(
            CHORUS_CLASSIFIER_TRAIN_DATA_FILE['seqRecur'])

    def __call__(self, dataset, idx):
        ssm_f, mels_f = getFeatures(dataset, idx)
        cliques = self._process(dataset, idx, ssm_f)
        mirexFmt = chorusDetection(
            cliques, ssm_f[0], mels_f, self.clf)
        return mirexFmt
    ...
    def _process(self, dataset, idx, ssm_f):
        tf = ExtractCliques()
        cliques_set = Preprocess_Dataset(
...
class AlgoSeqRecurSingle(AlgoSeqRecur):
    ...
    def __init__(self):
        ...
        self.clf = ChorusClassifier(
            CHORUS_CLASSIFIER_TRAIN_DATA_FILE['seqRecurS']
        )
    ...
    def getSingleChorusSection(self, mirexFmt, chorusDur=30):
        ...
        for idx in chorusIndices:
            ...
            intsec = np.sum([intervalIntersection(
                (begin, end), intervals[j]) for j in chorusIndices])
...
class AlgoSeqRecurBound():
    ...
    def __call__(self, dataset, idx):
        ...
        origCliques = ssmStructure_sr(ssm_f)
        cliques = buildRecurrence(origCliques, ssm_f[0])
        mirexFmt = matchCliqueLabel(
            ssm_f[0], boundaries, cliques, dataset, idx)
        return mirexFmt
...
class MsafAlgos():
    ...
    def __init__(self, boundaries_id):
        ...
        self.clf = ChorusClassifier(
            CHORUS_CLASSIFIER_TRAIN_DATA_FILE[boundaries_id])
...
class MsafAlgosBdryOnly():
    ...
    def _process(self, wavPath, ssm_f):
        ...
        for i in range(len(labels)):
            score = [blockSSM[i, j]
                     for j in chain(range(i), range(i+1, len(labels)))]
