...
class MusicHighlighter(object):
    def __init__(self):
        ...
        # place holder
        self.x = tf.compat.v1.placeholder(tf.float32, shape=[None, None, 128])
        self.pos_enc = tf.compat.v1.placeholder(
            tf.float32, shape=[None, None, self.dim_feature * 4]
        )
        self.num_chunk = tf.compat.v1.placeholder(tf.int32)
        self.build_model()
    ...
    def build_model(self):
        ...
        # Global max-pooling
        net = tf.squeeze(tf.reduce_max(input_tensor=net, axis=1), axis=1)
